...
-- Wed 10 Oct 2018 06:53:56 PM CDT
...
CREATE TABLE IF NOT EXISTS `TT`.`perfilInversion` (
  ...
  `usuario_nickname` VARCHAR(15) NOT NULL,
  PRIMARY KEY (`idperfilInversion`),
  INDEX `fk_perfilInversion_usuario1_idx` (`usuario_idUsuario` ASC, `usuario_nickname` ASC),
...
BEGIN
	UPDATE usuario SET nombre=_nombre , apellidoPaterno=_apellidoPaterno, apellidoMaterno=_apellidoMaterno, nickname=_nickname, correo=_correo, ocupacion=_ocupacion WHERE idUsuario=idUsuario ;
END$$
...
BEGIN
    DECLARE _result INT;
	DELETE FROM usuario WHERE nickname = _nickname;
    IF (SELECT idUsuario FROM usuario WHERE nickname = _nickname) = null THEN
		SET _result = 1;
	ELSE
		SET _result = 0;
...
BEGIN
    ...
    IF (SELECT saldo FROM tarjeta WHERE idTarjeta = _idTarjeta) = null THEN
		SET _result = 1;
	ELSE
		SET _result = 0;
    END IF;
...
BEGIN
	DECLARE _result INT;
	INSERT INTO flujo (fechaCorte, nombreFlujo, tipoFlujo, monto, periodicidad, idListaFlujos, idUsuario, nickname) 
    VALUES (_fechaCorte, _nombreFlujo, _tipoFlujo, _monto, _periodicidad, _idListaFlujos, _idUsuario, _nickname);
    IF (SELECT idFlujo FROM flujo WHERE  idListaFlujos = _idListaFlujos AND idUsuario = _idUsuario) <> null THEN
		SET _result = 1;
	ELSE
    	SET _result = 0;
    END IF;
...
DELIMITER $$
USE `TT`$$
CREATE PROCEDURE `updateFlujo` (IN _idFlujo INT, IN _nombreFlujo VARCHAR(20), IN _fechaCorte INT, IN _tipoFlujo TINYINT, IN _monto FLOAT, IN _estado TINYINT, IN _idListaFlujos INT, IN _idUsuario INT, IN _nickname VARCHAR(15))
BEGIN
	UPDATE flujo SET fechaCorte = _fechaCorte, nombreflujo=_nombreFlujo, tipoFlujo = _tipoFlujo, monto = _monto, estado = _estado, idListaFlujos = _idListaFlujos, idUsuario = _idUsuario, nickname = _nickname WHERE idFlujo = _idFlujo;
END$$
...
BEGIN
	SELECT fechCorte, saldo, comisionFija, porcentajexManejoCuenta, valorComisionFija FROM tarjeta, tarjetaDebito WHERE idUsuario = _idUsuario AND tarjeta.idTArjeta = tarjetaDebito.idTarjeta;
END$$
...
INSERT INTO `TT`.`usuario` (`idUsuario`, `nickname`, `nombre`, `apellidoPaterno`, `apellidoMaterno`, `ocupacion`, `correo`, `contrasena`, `idpartida`, `idcasaDeBolsa`) VALUES (2, 'vic', 'Victor', 'Amador ', 'Munoz', 'Estudiante', 'vic@gmail.com', 'vicvic', NULL, );
