...
<div class="wrap">
	<h2><?php printf(__("Manage Questions in %s", 'watu'), $exam_name); ?></h2>
		...
		&nbsp;|&nbsp; <a href="admin.php?page=watu_exam&quiz=<?php echo $_GET['quiz']?>&action=edit"><?php printf(__('Edit this %s', 'watu'), WATU_QUIZ_WORD)?></a> 
		&nbsp;|&nbsp; <a href="admin.php?page=watu_grades&quiz_id=<?php echo $_GET['quiz']?>"><?php _e('Manage Grades / Results', 'watu')?></a>
		&nbsp;|&nbsp; <a href="admin.php?page=watu_import_questions&quiz_id=<?php echo $_GET['quiz']?>"><?php _e('Import Questions', 'watu');?></a></p>
		...
		<p style="color:green;"><?php printf(__('To add this %s to your blog, insert the code ', 'watu'), WATU_QUIZ_WORD); ?> <input type="text" readonly size="8" onclick="this.select();" value="[WATU <?php echo $_REQUEST['quiz'] ?>]"> <?php _e('into any post or page.', 'watu') ?></p>
		...
		<?php _e('with ID (you can separate multiple IDs with comma):', 'watu')?> <input type="text" name="filter_id" value="<?php echo @$_GET['filter_id']?>" size="6"> 
		&nbsp;<br>
		<?php _e('and question contains (phrase):', 'watu');?> <input type="text" name="filter_contents" value="<?php echo @$_GET['filter_contents']?>">
		
		<input type="submit" value="<?php _e('Filter questions', 'watu')?>" class="button-primary">
		<input type="button" class="button" value="<?php _e('Clear filters', 'watu');?>" onclick="window.location='admin.php?page=watu_questions&quiz=<?php echo $_GET['quiz']?>';"></p>	
	</form>
		...
		<input type="submit" name="mass_delete" onclick="if(!confirm('<?php _e('Are you sure?', 'watu')?>')) return false;" value="<?php _e('Delete Selected', 'watu')?>" class="button">
